...
def main():
  ...
  # training process, which can refer to solver.py
  best_kl = 0.0
  best_mae = 1000
  for epoch in range(start_epoch,start_epoch+200):
    ...
    #test
    res_kl,res_mae = Solver.test(test_loader,epoch,device)
    print('In current epoch, mae: %.4f  cs: %.4f'%(res_mae, res_kl))
    if best_kl < res_kl:
      best_kl = res_kl
    if best_mae > res_mae:
      best_mae = res_mae
  ...
  print('In the total training process, the best cs is ',best_kl,' the best mae is ',best_mae)
